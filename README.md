...
如需让局域网其他设备访问后台，可以在后台设置里把监听地址改成 `0.0.0.0`，并建议同时配置管理员账号和密码。

默认 OneBot 回调路径：

- `/api/onebot?token=你的回调令牌`

在 Windows 单文件版里，直接双击 `exe` 会自动打开浏览器进入后台控制台；看门狗模式会自动关闭浏览器弹出，只保留后台常驻。

Windows 包同时提供一个原生桌面壳：

- `propagation_desktop.exe`
- 桌面壳会启动同目录下的 `propagation_bot.exe`
- 默认使用 `http://127.0.0.1:18080/` 作为内置后台地址
- 桌面壳提供启动 / 停止后台、打开网页后台、立即刷新和发送常用简报按钮

如果你想使用更完整的原生界面，可以运行：

- `propagation_qt_desktop.exe`
- Qt 前端默认使用 `http://127.0.0.1:18090/`
- 支持原生运行面板、刷新状态、PSK 图片预览、常用推送动作和前端日志
